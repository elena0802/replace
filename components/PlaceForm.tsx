...
export default function PlaceForm({
  ...
  const [recordHelperSuggestion, setRecordHelperSuggestion] = useState("");
  const [recordHelperError, setRecordHelperError] = useState("");
  const [isRecordHelperLoading, setIsRecordHelperLoading] = useState(false);
  ...
  const shouldShowRecordHelper = mode === "create";
  ...
  async function handleGenerateRecordSuggestion() {
    const placeName = values.name.trim();
    const memo = values.memory.trim();

    if (!placeName || !memo) {
      setRecordHelperError(recordHelperInputMessage);
      return;
    }

    setIsRecordHelperLoading(true);
    setRecordHelperError("");
    ...
    try {
      const response = await fetch("/api/ai/record-helper", {
        ...
        body: JSON.stringify({
          address: selectedPlace ? getPlaceAddress(selectedPlace) : values.region,
      ...
      if (!response.ok || typeof data.suggestion !== "string") {
        const errorMessage =
          typeof data.error === "string" ? data.error : recordHelperFallbackMessage;
      ...
      setRecordHelperSuggestion(suggestion);
    } catch (error) {
      const errorMessage =
      ...
      setRecordHelperError(errorMessage);
    } finally {
      setIsRecordHelperLoading(false);
    }
  }

  function handleApplyRecordSuggestion() {
    if (!recordHelperSuggestion) {
      return;
    }

    updateValue("memory", recordHelperSuggestion);
    setRecordHelperError("");
    window.setTimeout(() => {
        ...
        {shouldShowRecordHelper && (
          <div className="rounded-2xl border border-[#E5E0D8] bg-[#FFFDF8] p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button
                type="button"
                onClick={handleGenerateRecordSuggestion}
                disabled={isRecordHelperLoading || isSubmitting}
            ...
            {recordHelperSuggestion && (
              <div className="mt-4 rounded-2xl border border-[#E5E0D8] bg-white p-4">
                ...
                <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                  <button
                    type="button"
                    onClick={handleApplyRecordSuggestion}
